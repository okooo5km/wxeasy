...
/// 从当前工作目录 / <exe_dir> / $HOME/.wxeasy 加载配置
...
fn home_config_path(home_dir: &Path) -> PathBuf {
    home_dir.join(".wxeasy").join("config.json")
}

pub fn cli_dir() -> PathBuf {
    cli_home_dir().join(".wxeasy")
}
...
fn detect_db_dir_impl() -> Option<PathBuf> {
    ...
    // 1) %APPDATA%\Tencent\xwechat\config\*.ini
    //    旧版内容是绝对路径；4.1.10+ 常见令牌：`MyDocument:`（表示“文档”特殊文件夹）。
                ...
                for entry in entries.flatten() {
                    ...
                    if path.extension().map(|e| e == "ini").unwrap_or(false) {
                        if let Ok(content) = std::fs::read_to_string(&path) {
                            for root in resolve_windows_data_roots(content.trim()) {
                                collect_db_storage_under_xwechat_files(&root, &mut candidates);
                            }
                        }
                    }
                }
    ...
    // 2) 回退：直接扫 %USERPROFILE%\Documents\xwechat_files 与已知用户文档目录
    if let Some(home) = dirs::home_dir() {
        collect_db_storage_under_xwechat_files(&home.join("Documents"), &mut candidates);
    }
    if let Ok(userprofile) = std::env::var("USERPROFILE") {
        collect_db_storage_under_xwechat_files(
...
/// 解析 xwechat config ini 中的数据根：绝对路径或 `MyDocument:` 一类令牌。
#[cfg(target_os = "windows")]
fn resolve_windows_data_roots(token: &str) -> Vec<PathBuf> {
    ...
    // 4.1.x：`MyDocument:` / `MyDocuments:` / `Documents:`
    let lower = t.to_ascii_lowercase();
    if lower.starts_with("mydocument")
        || lower.starts_with("mydocuments")
        || lower == "documents:"
        || lower.starts_with("documents:")
    {
        ...
        // 也可能是“文档”中文目录名（部分系统显示名）
        if let Some(home) = dirs::home_dir() {
            roots.push(home.join("文档"));
        }
        return roots.into_iter().filter(|p| p.is_dir()).collect();
    }
    Vec::new()
}

#[cfg(target_os = "windows")]
fn collect_db_storage_under_xwechat_files(data_root: &Path, out: &mut Vec<PathBuf>) {
    // data_root 可能是“文档”目录本身，其下是 xwechat_files；也可能已经是 xwechat_files
    let pattern = if data_root
        .file_name()
        .and_then(|s| s.to_str())
        .map(|s| s.eq_ignore_ascii_case("xwechat_files"))
        .unwrap_or(false)
    {
        data_root.to_path_buf()
    } else {
        data_root.join("xwechat_files")
    };
    if !pattern.is_dir() {
        return;
    }
...
mod tests {
    ...
    fn config_path_prefers_cwd_over_exe_and_home() {
        ...
        fs::create_dir_all(home.join(".wxeasy")).unwrap();
