...
//!
//! 背景：`all_keys.json` 是 `wxeasy init` 时的快照。WeChat 在 daemon 启动后随时可能创建
...
pub struct Meta {
    ...
    /// 非空 ⇒ `wxeasy init` 之后 WeChat 又分裂了新分片 → 必须重跑 `wxeasy init`。
...
pub enum MetaStatus {
    ...
    PossiblyStale,
    /// 最强信号：磁盘上出现 daemon 不认识的新分片，通常必须重跑 `wxeasy init --force`。
...
mod tests {
    ...
    fn tempdir() -> std::path::PathBuf {
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .unwrap()
            .as_nanos();
        let p = std::env::temp_dir().join(format!("wxeasy-meta-test-{}-{}", pid, nanos));
