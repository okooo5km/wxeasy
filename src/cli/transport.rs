    ...
    match pid_file {
        Some(pid_file) => {
            ...
            if daemon_alive && !belongs {
                bail!(
                    "daemon 正在运行，但 {} 指向的 PID {} 无法确认属于当前 wxeasy-daemon",
                    pid_path.display(),
                    pid_file.pid
                );
            }
...
/// 启动 daemon 前检查 `~/.wxeasy/` 可写，给出比"超时"更明确的错误。
///
/// 典型坑：旧版本 `sudo wxeasy init` 把目录留成 root 属主，非 root 的 daemon
    ...
    match std::fs::File::create(&probe) {
        ...
        Err(e) if e.kind() == std::io::ErrorKind::PermissionDenied => {
            ...
            if cfg!(unix) {
                bail!(
                    "无法写入 {dir}（权限不足）\n\n\
                     这通常是老版本的 `sudo wxeasy init` 把目录属主留成了 root。\n\
                     修复：\n\n    \
...
/// 启动 daemon 进程（自身二进制，设置 WXEASY_DAEMON_MODE=1）
fn start_daemon() -> Result<()> {
    let exe = std::env::current_exe().context("无法获取当前可执行文件路径")?;
    let child_pid: u32;

    // 预检：当前用户是否能写 ~/.wxeasy/。如果不能，给出可操作的错误信息，
