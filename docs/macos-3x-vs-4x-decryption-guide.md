...
> **Windows 读者**：Windows 微信 **4.1.10+** 的内存提钥限制、page1 强校验与 `all_keys.json` 复用策略见 [windows-4.1.10-keys-and-reuse.md](./windows-4.1.10-keys-and-reuse.md)（与本文 macOS 内存扫描路径不同）。
