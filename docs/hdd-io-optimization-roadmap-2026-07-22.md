...
## 0. 结论速览（2026-07-22 更新：方案 1/2/3/5/6 已实施）

| # | 方案 | 验证结论 | 复杂度 | 状态 |
|---|------|---------|--------|------|
| 1 | `_SORTSEQ` 索引改写全部 `create_time` 查询 | viable-with-changes | 低 | ✅ 已落地（`24717de` + 选择性门控 `2c35a1a`） |
| 2 | `build_wal_index` 大块顺序读 | viable-with-changes | 低 | ✅ 已落地（`cf23e9a`） |
| 3 | WAL 帧索引增量追扫（跨重建缓存） | viable | 低 | ✅ 已落地（`2411ca5`） |
| 4 | schema 页电梯预读（工作集记录 + 排序合并预读） | viable | 低-中 | T2（现场数据后） |
| 5 | ShardRouteCache 持久化 | viable-with-changes | 中 | ✅ 已落地（`74a0871`，决策点 1 已拍板选快；**MAX 缓存扩展主动放弃**，见 §4.2 附注） |
| 6 | contact.db 派生 Names 持久化（加密） | viable-with-changes | 低 | ✅ 已落地（`97d2347`，决策点 2 已拍板选加密） |
...
### 实施后实测（2026-07-22，开发机、暖 OS 缓存、真实微信数据）

对照 baseline = `a3a6d96`。规模：1 分片 100MB、9.9 万行、77 表、WAL 4MB、
contact.db 6.7MB、7493 联系人。**暖缓存口径**——收益主要来自「免全表扫 +
免全表逐页 AES 解密」；冷 HDD 上（免随机 seek）差距只会更大，真·冷盘
数字仍需目标机器重启后按 §8 清单采集。

| 场景 | baseline | 实施后 |
|------|----------|--------|
| `history` 50 条（活跃群，首查） | ~400ms | **~160ms** |
| `history` 稳态 | ~395ms | **~100ms** |
| `new-messages`（~90 会话变化/轮） | 390~600ms | **~175-200ms** |
| daemon 重启后路由恢复 | 全量重扫 | 从磁盘恢复（日志确认） |
| daemon 重启后联系人加载 | 全表扫 ~380ms | 加密缓存命中（7493 条，日志确认；contact.db 10 分钟内被写过则按设计 miss 冷扫） |

正确性验证：`cargo test` 217 通过；oracle 真实数据对拍通过（VFS 合并视图
vs full_decrypt 逐项一致）；77 表 × 1245 项新旧 SQL 逐行对拍零差异
（含 NULL/0 sort_seq 与时间倒挂形态）。

**实测教训（已修复 + 写进防回归测试）**：`_SORTSEQ` 下推初版在「调用方
state 陈旧、since 覆盖整表」时，索引范围扫 + 逐行回表比顺序全表扫慢 3 倍
（~400ms → ~1150ms/轮）。索引只在匹配集小的时候赢。修复：covering index
选择性探针（`LIMIT 1 OFFSET T`，T = max(1024, 4×limit)），超阈值回退
全表扫（= baseline 行为）。教训通用化：**任何「索引下推」优化必须带
选择性门控，否则宽范围场景必然回归**。
...
## 7. 需要拍板的决策点（已全部拍板，2026-07-22）

| # | 决策 | 选项 | 结果 |
|---|------|------|------|
| 1 | 路由持久化 vs 首轮全量重置护栏（§4.2） | A 要冷启动收益，接受 LOW 残余窗口寿命延长（写进 §4）；B 不做方案 5 | ✅ **选 A（要快）**。取舍已备案在 `cache.rs::RouteCacheFile` 文档与本文件 §4.2 |
| 2 | Names 缓存明文（§4.3） | A 加密缓存文件（推荐）；B 接受明文；C 不做 | ✅ **选 A（加密）**。AES-256-CBC + HMAC-SHA256，密钥复用 contact enc_key，见 `names_cache.rs` |
| 3 | T1 三刀是否即刻开工 | 是 / 否 | ✅ 是，已全部落地 |

方案 5 的 per-table MAX 缓存扩展**主动放弃**：方案 1 落地后
`MAX(create_time)` 已是「已开连接上的 3 页索引下潜」，而所有调用路径
（history/search/stats）在拿到 MAX 后都会紧接着在同一连接上查消息——
缓存 MAX 买不到任何 IO，还要背世代号回写纪律，不值。
